...
class Orchestator(OrchestatorCommon):
    ...
    def upload_bot(self, process, name, bot_name, db, project):
        data = {'name': name, 'startbot': bot_name,
                'process_id': process, 'project_id': project}

        file_handle = open(db, 'rb')
        response = self.request(
            "post", "/api/process/robot/update/data", data, self.headers['FORM'], {"file": file_handle})
        file_handle.close()
        return response

    def get_process_executions(self, filter_callback:function=None):
        
        response = self.request("post", "/api/process/list", None, self.headers['JSON'])
        if filter_callback:
            response = filter(filter_callback, response)
        return response

    def get_projects(self, filter_callback:function=None):
